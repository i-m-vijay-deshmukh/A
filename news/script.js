...
async function getUserCountry() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async () => {
            ...
            console.warn("Geolocation permission denied, falling back to IP lookup...");
            try {
                const geoResponse = await fetch(`https://ipapi.co/json/`);
                const geoData = await geoResponse.json();
                userCountry = geoData.country_code.toLowerCase();
...
async function fetchNews() {
    ...
    try {
        ...
        if (!data.articles || data.articles.length === 0) {
            newsContainer.innerHTML += "<p>No more agriculture news available.</p>";
            return;
        }
        ...
        newsContainer.innerHTML += "<p>⚠️ Error fetching more agriculture news. Please try again.</p>";
        isFetching = false;
    }
}
